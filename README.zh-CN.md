...
当前文档对应 **1.8.0**，最低需要 **Obsidian 1.8.7**。它以完全没有 S3、R2 使用经验的读者为主要对象；如果你已经熟悉这些概念，可以直接阅读[目录策略](#目录策略四种模式)、[与 Agent 和整理插件配合](#与-agent-和整理插件配合)或[三方一致性校对](#三方一致性校对)。
...
**1.8.0 新变化：**“一键下载云端图片至本地镜像”改为先预览、再确认，仅补齐缺失图片，绝不覆盖已有文件。切换本地链接时也不再用同名不同扩展名图片代替精确镜像。最低应用版本与 1.7.3 相同。详细操作见下文“小云朵菜单”。
...
| 一键下载云端图片至本地镜像 | `staging` / `managed` 引用 | 否 | 带签名 GET，只读 | 确认后仅创建缺失文件 | 先预览：一致保留、冲突保留；补齐后回读校验，不覆盖已有文件 |
...
1. 点击小云朵 → **一键下载云端图片至本地镜像**，等待只读预览完成。此时没有写入文件。
2. 查看状态：**本地缺失**可补齐；**内容一致**无需处理；**内容冲突**会保留本地文件；**云端不存在**不能补齐；失败项会给出路径、权限、内容校验等原因类别。
3. 点击 **仅补齐缺失文件**。这会处理所有页面的缺失项，不仅是当前页。程序重新读取云端内容，与预览时的 SHA-256 比较；云端版本变化、本地已被其他工具创建或引用变化时跳过，不覆盖。
4. 结果每页显示 50 项，可翻页或点击 **复制完整报告（含私人路径）**。报告不含配置和访问密钥，但包含笔记路径、对象键及哈希，发给他人前请检查隐私。
5. 点击 **取消** 或关闭窗口可停止后续操作；修改存储／目录设置、停用插件也会停止任务。已发出的请求或写入可能完成，已经补齐的文件会保留。关闭后重新运行可获得新预览。

**流量提醒：**预览会完整读取每张引用图片并计算哈希，即使云端没有哈希元数据也能比较真实内容；确认补齐时会再次下载缺失图片。因此预览不是零流量操作，可能产生服务商费用。按对象串行处理，不同时缓存全库图片正文；单个大文件仍需要相应内存。

**范围边界：**只处理 `staging` / `managed` 笔记可识别的引用，不是备份整个 Bucket。不会修复笔记路径、覆盖冲突或删除孤儿文件；`verify` / `ignore` 规则不变。若是冲突，请先备份并人工确定正确版本，本版本不提供强制覆盖按钮。Windows 无法安全表示的路径会报告失败，不会悄悄改名后写入错误位置。

实现和验证记录见 [1.8.0 镜像恢复说明](docs/MIRROR-RESTORE-1.8.0.md)。取消和设置快照目前仅接入此下载命令，并不代表所有小云朵命令已统一改造。
...
| 版本 | 重点 | 状态 |
| --- | --- | --- |
| 1.8.0 | 下载前只读预览、仅补齐缺失镜像、取消与设置快照、分页和完整报告；本地切换只认精确扩展名 | 当前文档与当前代码版本 |
| 1.7.3 | 精确替换链接、特殊字符与标题解析、后台失败持久重试、目录规则校验应用；最低应用版本纠正为 1.8.7 | 历史可靠性补丁 |
| 1.7.2 | 将默认 README 改为完整英文指南，并把详细中文手册独立保留，消除社区目录语言审核警告 | 历史版本 |
| 1.7.1 | 修复社区目录审核项，并把官方 Obsidian ESLint 规则和描述规范纳入本地检查及 CI | 历史版本 |
| 1.7.0 | 四种目录策略、启动补处理、签名下载、历史 URL、事务式路径迁移、版本化延迟删除、三方一致性校对与 CI/Release 防漂移 | 历史版本；因描述规范错误已被社区目录撤下，请升级到当前版本 |
...
S3 Image Sync Pro 1.8.0 requires Obsidian 1.8.7 or later. It is a plugin for Cloudflare R2, AWS S3, MinIO, and other S3-compatible object stores. It manages three related views of an image: the Markdown reference, the local mirror inside the vault, and the cloud object.
