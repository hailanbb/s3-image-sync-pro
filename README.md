...
当前版本为 **1.6.7**，最低支持 Obsidian **1.6.6**。历史兼容关系见 [`versions.json`](versions.json)。
...
> 当前版本没有实现 `{yyyy}`、`{MM}`、`{dd}` 等日期变量。请以本表为准。
...
| 自动转存网络图片 | 关闭 | 笔记创建或修改后等待约 5 秒，下载外部图片并转存。开启后建议重载插件。 |
...
| 删除笔记时同步删除云端图片 | 关闭 | 高风险；删除笔记或移除受管链接时会尝试删除云端对象和镜像，不检查跨笔记引用。 |
...
> 当前 v1.6.7 的粘贴/拖拽自动上传属于编辑器主动操作，不受“不处理的笔记路径”拦截。若某目录必须绝对禁止上传，请关闭该开关，或不要在该目录笔记中粘贴/拖拽图片。
...
> 当前 v1.6.7 的定时自动扫描只处理 `90-笔记系统/92-附件` 下的附件，设置页暂时没有修改该目录的入口。如果你的附件不在这里，请使用“扫描当前笔记图片”。
...
- 当前版本主要重写云端 URL。笔记使用本地链接时，建议先切换为云端，再运行重同步；完成并补齐镜像后再切回本地。
...
开启后，当前实现不仅会在删除笔记时清理，还会在检测到受管图片链接从笔记中移除时，尝试删除对应云端对象和本地镜像。它**不会检查其他笔记是否仍引用同一张图片**。

只有在你能保证“一张图片只属于一篇笔记”时才建议开启。共享图片、Logo、封面和模板资源不适合使用该策略。

另外，在推荐的按笔记路径镜像结构下，删除笔记会清理对应的本地镜像目录；“不处理的笔记路径”中的笔记会跳过这一行为。

## 当前限制

- 插件以笔记中的链接为索引，不会主动列出整个 Bucket，也不会自动发现没有被笔记引用的孤立对象。
- 路径同步依赖 `{notedir}/{notename}/...` 结构；改为纯哈希或日期路径后，移动和重同步能力会受限。
- Vault 根目录笔记按默认模板生成的对象键只有“笔记名/文件名”两段，当前全量启动检查和重同步不会处理这类两段式键。
- 链接切换、下载和路径重写主要针对标准 Markdown 图片语法 `![说明](地址)`。
- 全量重新同步主要改写云端 URL；本地模式笔记建议先切到云端再执行。
- “不处理的笔记路径”当前不拦截用户主动触发的粘贴/拖拽自动上传。
...
生产构建会把 WebP 所需 WASM 资源打包进 `main.js`，Release 安装不需要额外下载 `.wasm` 文件。
